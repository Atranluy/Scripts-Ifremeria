...
### Adapted model for folded FS from Rougement et al 2017 plus incorporing IM2N2m and IM2N2mG and growth (G) only during migration phase.
### 
import numpy
import dadi
...
def IM(params, ns, pts):
    ...
    """
    nu1,nu2,nuA,Ta,Ts,m12,m21 = params
...
def IMa(params, ns, pts):
    ...
    ns = ns
    params = (s,nu1,nu2,T,m12,m21)
    Isolation-with-migration model with ancestral pop split divided in s and 1-s size
    s: Size of pop 1 after split. (Pop 2 has size 1-s.)
    nu1: Final size of pop 1.
    nu2: Final size of pop 2.
    T: Time in the past of split (in units of 2*Na generations) 
...
def IMG(params, ns, pts):
    """
    ns = ns
    nu1, nu2,nuA,b1, b2,Ta, Ts, m12, m21= params
    Isolation-with-migration model with exponential pop growth.
    nu1: Final size of pop 1.
    nu2: Final size of pop 2.
	b1: growth parameter over Ts for pop1
	b2: growth parameter over Ts for pop2
    T: Time in the past of split (in units of 2*Na generations) 
    m12: Migration from pop 2 to pop 1 (2*Na*m12)
    m21: Migration from pop 1 to pop 2
    pts: Number of grid points to use in integration.
...
def IM2N(params, ns, pts):
    nu1, nu2,nuA, hrf,Ta,Ts,m12, m21, Q = params

    """
    Model IM with heterogenous effective population size (with 2 classes of loci shared by the two populations = Hill-Robertson effects) considering the linked selection 
    ...
    """
	### spectrum for ancestral population and split
    xx = dadi.Numerics.default_grid(pts)
    phi = dadi.PhiManip.phi_1D(xx)
    phi = dadi.Integration.one_pop(phi, xx,Ta, nu = nuA)
    phi = dadi.PhiManip.phi_1D_to_2D(xx, phi)
...
def IM2NG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta,Ts, m12, m21, Q = params

    """
    Model IM with populaiton growth and heterogenous effective population size (with 2 classes of loci shared by the two populations = Hill-Robertson effects)
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
...
def IM2m(params, ns, pts): 
    nu1, nu2,nuA,Ta,Ts, m12, m21, me12, me21, P = params
    """
...
def IM2mG(params, ns, pts): 
    nu1, nu2,nuA, b1, b2,Ta, Ts, m12, m21, me12, me21, P = params
    """
    Model with migration during the divergence with two type of migration.
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
...
def IM2N2m(params, ns, pts): 
    nu1, nu2,nuA, hrf,Ta,Ts,m12, m21, me12, me21, P, Q = params
    """
    Model IM of semi permeability with 2 migration rates and low recombining regions (hrf)
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
    ...
    phi = dadi.Integration.one_pop(phi, xx,Ta, nu = nuA)
    phi = dadi.PhiManip.phi_1D_to_2D(xx, phi)
	####### calculate the non reduce migraiton rate spectrum
    phiN = phi
    ...
    ####### Calculate spectrum of low-recombining regions
    # phi for the equilibrium ancestral population
    philr = phi
...
def IM2N2mG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta,Ts ,m12, m21, me12, me21, P, Q = params
    """
    Model IM of semi permeability with 2 migration rates , low recombining regions and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
...
def SIG(params, ns, pts):
    ...
    pts: Number of grid points to use in integration.
    """
    nu1, nu2,nuA, b1, b2,Ta, Ts = params
...
def SI2N(params, ns, pts):
    nu1, nu2,nuA, hrf,Ta ,Ts, Q = params
    """
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
...
def SI2NG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta, Ts, Q = params
...
.
    ...
    pts: Number of points to use in grid for evaluation.
    """
    nu1,nu2,nuA,Ta,Tam,Ts,m12,m21 = params
...
def AMG(params, ns, pts):
    ...
    pts: Number of grid points to use in integration.
    """
    nu1, nu2,nuA, b1, b2,Ta, Tam,Ts, m12, m21= params
...
def AM2N(params, ns, pts):
    nu1, nu2,nuA, hrf,Ta, Tam,Ts,m12, m21, Q = params
...
def AM2NG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta,Tam,Ts, m12, m21, Q = params
...
def AM2m(params, ns, pts): 
    nu1, nu2,nuA,Ta,Tam,Ts, m12, m21, me12, me21, P = params
    """
    Model with migration during the divergence with two type of migration.
...
def AM2mG(params, ns, pts): 
    nu1, nu2,nuA, b1, b2,Ta, Tam,Ts, m12, m21, me12, me21, P = params
    """
    Model with migration during the divergence with two type of migration.
...
def AM2N2m(params, ns, pts): 
    nu1, nu2,nuA, hrf,Ta, Tam,Ts, m12, m21, me12, me21, P, Q = params
    """
...
def AM2N2mG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta,Tam,Ts,m12, m21, me12, me21, P, Q = params
    """
    Model of semi permeability with split, complete isolation, followed by secondary contact with 2 migration rates
...
.
    ...
    pts: Number of points to use in grid for evaluation.
    """
    nu1,nu2,nuA,Ta,Ts,Tsc,m12,m21 = params
...
def SCG(params, ns, pts):
    ...
    pts: Number of grid points to use in integration.
    """
    nu1, nu2,nuA, b1, b2,Ta, Ts,Tsc, m12, m21= params
    xx = dadi.Numerics.default_grid(pts)
...
def SC2N(params, ns, pts):
    nu1, nu2,nuA, hrf,Ta, Ts, Tsc,m12, m21, Q = params
...
def SC2NG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta, Ts, Tsc, m12, m21, Q = params
...
def SC2m(params, ns, pts): 
    nu1, nu2,nuA,Ta,Ts,Tsc, m12, m21, me12, me21, P = params
    """
    Model with migration during the divergence with two type of migration.
...
def SC2mG(params, ns, pts): 
    nu1, nu2,nuA, b1, b2,Ta, Ts, Tsc, m12, m21, me12, me21, P = params
    """
    Model with migration during the divergence with two type of migration.
...
def SC2N2m(params, ns, pts): 
    nu1, nu2,nuA, hrf,Ta, Ts, Tsc, m12, m21, me12, me21, P, Q = params
    """
    Model of semi permeability with split, complete isolation, followed by secondary contact with 2 migration rates
...
def SC2N2mG(params, ns, pts):
    nu1, nu2,nuA, b1, b2, hrf,Ta, Ts, Tsc,m12, m21, me12, me21, P, Q = params
    """
    Model of semi permeability with split, complete isolation, followed by secondary contact with 2 migration rates
