...
def IMG(params, ns, pts):
    ...
    phi = dadi.Integration.two_pops(phi, xx, Ts, nu1_func, nu2_func,m12=m12, m21=m21)
    fs = dadi.Spectrum.from_phi(phi, ns, (xx,xx))
    return fs    
...
def IM2NG(params, ns, pts):
    ...
    """
    Model with split with migration, heterogenous effective population size (with 2 classes of loci shared by the two populations = Hill-Robertson effects) and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    ...
    #### Calculate the pectrum in normally-recombining region
    phinr = phi
...
def IM2m(params, ns, pts): 
    ...
    O: The proportion of accurate orientation, only in unfolded FS
    n1,n2: Size of fs to generate.
    pts: Number of points to use in grid for evaluation.
    """
    # Define the grid we'll use
...
def IM2N2m(params, ns, pts): 
    ...
    """
    Model of semi permeability with split and migration with 2 migration rates and linked selection 
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
...
def IM2N2mG(params, ns, pts):
    ...
    """
        Model of semi permeability with split and migration with 2 migration rates, linked selection and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
    ...
    #### Calculate the heterogenous migration spectrum
    phiI = phi
    ...
    #### Calculate the linked selection spectrum
    philr = phi
    # Now do the divergence event
    # We keep the population sizes after the split and isolation to nu1 and nu2 and set the migration rate to m12 and m21
    bnu1hrf_func = lambda t: (nu1 * b1**(t/Ts)) * hrf
...
def SI(params, ns, pts):
    """
    vicariance model (i.e Strict Isolation) 
    ns = ns
...
def SIG(params, ns, pts):
    """
    Strict isolation with population growth
    ns = ns
    params = (s,nu1,nu2,T,m12,m21)
...
def SI2N(params, ns, pts):
    ...
    """
    Strict Isolation with linked selection 
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    Ts: The scaled time between the splitÂ and present (in units of 2*Na generations).
...
def SI2NG(params, ns, pts):
    ...
    """
    Strict Isolation with linked selection and population growth
...
def AM(params, ns, pts):
    ...
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
    m21: Migration from pop 1 to pop 2.
    Tam: The scaled time between the split and the strict isolation (in units of 2*Na generations).
    Ts: The scale time between the ancient migration and present.
    n1,n2: Size of fs to generate.
    pts: Number of points to use in grid for evaluation.
    """
...
.
    ...
    pts: Number of points to use in grid for evaluation.
    """
    s,nu1,nu2,Tam,Ts,m12,m21 = params
...
def AMG(params, ns, pts):
    ...
    pts: Number of points to use in grid for evaluation.
    """
    nu1, nu2, b1, b2, Tam,Ts, m12, m21= params
    ...
    phi = dadi.Integration.two_pops(phi, xx, Tam, nu1_func, nu2_func,
                               m12=m12, m21=m21)
    ######  after the ancien migration period we set population size to nu1 *b1 and nu2 *b2 to avoid simulating an instantaneous bottleneck or expansion in population size just after the migration phase
    nu1_b = nu1 * b1
    nu2_b = nu2 * b2   
...
def AM2N(params, ns, pts):
    nu1, nu2, hrf, Tam, Ts,m12, m21, Q = params

    """
    Ancien Migration model with linked selection
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
...
def AM2NG(params, ns, pts):
    nu1, nu2, b1, b2, hrf, Tam, Ts, m12, m21, Q = params

    """
    Ancient migration model with linked selection and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
...
def AM2m(params, ns, pts): 
    nu1, nu2,Tam,Ts, m12, m21, me12, me21, P = params
    """
    Ancient migration model with heterogenous migration
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
    m21: Migration from pop 1 to pop 2.
...
def AM2mG(params, ns, pts): 
    nu1, nu2, b1, b2, Tam, Ts, m12, m21, me12, me21, P = params
    """
    Ancient migration model with heterogenous migration and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
...
def AM2N2m(params, ns, pts): 
    nu1, nu2, hrf, Tam, Ts, m12, m21, me12, me21, P, Q = params
    """
    Ancient migration model with heterogenous migration and linked selection
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
...
def AM2N2mG(params, ns, pts):
    nu1, nu2, b1, b2, hrf, Tam, Ts,m12, m21, me12, me21, P, Q = params
    """
    
    Ancient migration model with heterogenous migration, linked selection and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
    ...
    #### Calculate heterogenous migration spectrum
    phiI = phi
    # We keep the population sizes change after the split to bnu1 and bnu2 and set the migration rates to me12 and me21
    phiI = dadi.Integration.two_pops(phiI, xx, Tam, bnu1_func, bnu2_func, m12=me12, m21=me21)
    nu1_b = nu1 * b1
    ...
    #### Calculate linked selection spectrum 
    philr = phi
    # We keep the population sizes after the split and isolation to nu1 and nu2 and set the migration rate to m12 and m21
    bnu1hrf_func = lambda t: (nu1 * b1**(t/Tam)) * hrf
    bnu2hrf_func = lambda t: (nu2 * b2**(t/Tam)) * hrf
...
def SCG(params, ns, pts):
    """
    Secondary contact model with population growth
    ns = ns
...
def SC2N(params, ns, pts):
    ...
    """
    Secondary contact model with heterogenous effective population size (with 2 classes of loci shared by the two populations = Hill-Robertson effects)
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
    ...
    ##### calculate heterogenous effective population size spectrum due to linked selection 
    philr = phi
    # We set the population sizes after the split to hrf*nu1 and hrf*nu2 and the migration rates to m12 and m21 
    philr = dadi.Integration.two_pops(philr, xx, Ts, nu1*hrf, nu2*hrf, m12=0, m21=0)
...
def SC2NG(params, ns, pts):
    ...
    """
    Secondary contact model with heterogenous effective population size and population growth (with 2 classes of loci shared by the two populations = Hill-Robertson effects)
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
    ...
    #### calculate neutral spectrum (ie normally-recombining)
    phinr=phi
    # We start the population size change after the split independantly in each population and set the migration rates to m12 and m21
...
def SC2m(params, ns, pts): 
    ...
    """
    Secondary contact model with heterogenous migration 
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
    m21: Migration from pop 1 to pop 2.
    ...
    ##### calculate the neutrally migration spectrum
    phiN=phi
    # We start the population size change after the split, independently in each population, and set the migration rates to m12 and m21
    phiN = dadi.Integration.two_pops(phiN, xx, Ts, nu1, nu2, m12=0,m21=0)
    ...
    ##### calculate the heterogenous migration (barrier-loci/ genomic island)
    phiI = phi
    # We start the population size change after the split, independently in each population, and set the migration rates to me12 and me21
    phiI = dadi.Integration.two_pops(phiI, xx, Ts, nu1, nu2, m12=0, m21=0)
...
def SC2mG(params, ns, pts): 
    ...
    """
    Secondary contact model with heterogenous migration and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
    ...
    ### Calculate the genomic island spectrum (heterogenous migration)
    phiI = phi
    # We start the population size change after the split, independently in each population, and set the migration rates to me12 and me21
    phiI = dadi.Integration.two_pops(phiI, xx, Ts,  nu1, nu2, m12=0, m21=0)
...
def SC2N2m(params, ns, pts): 
    ...
    """
    Secondary contact model with linked selection and heterogenous migration
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    hrf: Hill-Robertson factor, i.e. the degree to which Ne is locally reduced due to the effects of background selection and selective sweep effects
    m12: Migration from pop 2 to pop 1 (2*Na*m12).
...
def SC2N2mG(params, ns, pts):
    ...
    """
    Secondary contact model with linked selection, heterogenous migration and population growth
    nu1: Size of population 1 after split.
    nu2: Size of population 2 after split.
    b1: Population growth coefficient of population 1
    b2: Population growth coefficient of population 2
